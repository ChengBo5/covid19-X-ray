...
def dpn92(pretrained=False, test_time_pool=False, **kwargs):
    ...
    if pretrained:
        ...
        model.load_state_dict(torch.load("model_result/gcnet_train_30_128/gcnet.pt"))
        print('load modle gcnet.pt')
...
class DualPathBlock(nn.Module):
    def __init__(
            self, in_chs, num_1x1_a, num_3x3_b, num_1x1_c, inc, groups, inplanes,block_type='normal', b=False):
        ...
        self.gcnet=GlobalContextBlock(inplanes, ratio=0.25)
...
class DPN(nn.Module):
    def __init__(self, small=True, num_init_features=64, k_r=96, groups=32,
                 b=False, k_sec=(3, 4, 20, 3), inc_sec=(16, 32, 24, 128),
                 num_classes=3, test_time_pool=False,attention=False):
        ...
        blocks['conv2_1'] = DualPathBlock(num_init_features, r, r, bw, inc, groups,64,'proj', b)
        in_chs = bw + 3 * inc
        for i in range(2, k_sec[0] + 1):
            blocks['conv2_' + str(i)] = DualPathBlock(in_chs, r, r, bw, inc, groups,64, 'normal', b)
            in_chs += inc
        ...
        inc = inc_sec[1]
        r = (k_r * bw) // (64 * bw_factor)
        blocks['conv3_1'] = DualPathBlock(in_chs, r, r, bw, inc, groups,128, 'down', b)
        in_chs = bw + 3 * inc
        for i in range(2, k_sec[1] + 1):
            blocks['conv3_' + str(i)] = DualPathBlock(in_chs, r, r, bw, inc, groups,128,'normal', b)
            in_chs += inc
        ...
        inc = inc_sec[2]
        r = (k_r * bw) // (64 * bw_factor)
        blocks['conv4_1'] = DualPathBlock(in_chs, r, r, bw, inc, groups,256, 'down', b)
        in_chs = bw + 3 * inc
        for i in range(2, k_sec[2] + 1):
            blocks['conv4_' + str(i)] = DualPathBlock(in_chs, r, r, bw, inc, groups,256,'normal', b)
            in_chs += inc
        ...
        inc = inc_sec[3]
        r = (k_r * bw) // (64 * bw_factor)
        blocks['conv5_1'] = DualPathBlock(in_chs, r, r, bw, inc, groups,512,'down', b)
        in_chs = bw + 3 * inc
        for i in range(2, k_sec[3] + 1):
            blocks['conv5_' + str(i)] = DualPathBlock(in_chs, r, r, bw, inc, groups,512,'normal', b)
            in_chs += inc
